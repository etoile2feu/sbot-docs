...
Par défaut, Sbot configure automatiquement les permissions des slashs commandes (par exemple tout le monde a accès à /8ball mais seules les personnes ayant la permission gérer les messages ou étant admin ont accès à /clear-messages). Néanmoins tu peux modifier cela très facilement.

\*CE TUTO MARCHE AVEC TOUS LES BOTS EN SLASHS COMMANDES, ET PAS SEULEMENT SBOT !\*

## Il faut être sur ordinateur ou navigateur

Vas dans les paramètres du serveur, puis **intégrations**. Tu devrais voir un quelque chose comme cela :
...
Dans ce menu, tu retrouveras les slashs commandes de Sbot:

Outre le fait que tu peux limiter les channels où l'on peut utiliser les commandes, tu peux modifier qui peut accéder à une commande.
...
Sur cette fenêtre, tu peux ajouter des rôles ou des membres autorisés à accéder à la commande, ajouter des salons où la commande peut être exécutée, et afficher les permissions de la commande actuelle.
...
Ici tu peux sélectionner tous les rôles ou membres qui auront désormais accès à la commande (remarque: cocher @everyone autorisera tous les membres à accéder à la commande). Puis clique sur Ajouter. Si je veux que "SALUT À TOUS ET À TOUTES" seul ait accès à la commande par exemple, je sélectionne.
...
Et voilà ! Désormais seul les rôles et les utilisateurs que tu as sélectionné auront accès à la commande. Tu verras le label "A des dérogations", indiquant que l'opération a réussi.
